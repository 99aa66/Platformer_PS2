...
public class Head1 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        //Vector2 StickPosdifference = (StickPos - (Vector2)playerPos.transform.position).normalized;
...
//    Vector2 mousePos = new Vector2(cam.ScreenToWorldPoint(Input.mousePosition).x, cam.ScreenToWorldPoint(Input.mousePosition).y);  // Récupération de la position de la souris
//    Vector2 difference = (mousePos - (Vector2)playerPos.transform.position).normalized; // Calcul de la direction entre la position du joueur et celle de la souris
